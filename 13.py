...
def apply_xai_correction(predicted_mask, heatmap, threshold=0.7):
    """
    Applies XAI-based correction to the predicted mask using the heatmap.
    Args:
        predicted_mask: Original predicted mask (tensor)
        heatmap: XAI heatmap (tensor)
        threshold: Heatmap threshold for correction
    Returns:
        Corrected mask (PIL Image)
    """
    # Normalize heatmap
    heatmap_norm = (heatmap - heatmap.min()) / (heatmap.max() - heatmap.min())
    
    # Identify uncertain regions (where heatmap values are high)
    uncertain_regions = heatmap_norm > threshold
    
    # For uncertain regions, we'll set them to the most common class in the non-uncertain regions
    if uncertain_regions.any():
        certain_regions = ~uncertain_regions
        if certain_regions.any():
            # Get the most frequent class in certain regions
            mode_class = torch.mode(predicted_mask[certain_regions])[0]
            # Apply correction
            predicted_mask[uncertain_regions] = mode_class
    
    return transforms.ToPILImage()(predicted_mask.byte())

def calculate_xai_improvement(model, images, masks):
    """
    Calculates the accuracy improvement after applying XAI-based corrections.
    Returns:
        original_accuracy: Accuracy without XAI
        corrected_accuracy: Accuracy with XAI corrections
    ...
    original_accuracy = calculate_accuracy(predicted_masks, masks)
    corrected_accuracy = calculate_accuracy(corrected_masks, masks)
    
    improvement = ((corrected_accuracy - original_accuracy) / original_accuracy) * 100
    
    return original_accuracy, corrected_accuracy, improvement
...
if __name__ == "__main__":
    ...
    output_dir = r"C:\Users\Lenovo\Dropbox\PC\Desktop\Real-time-Semantic-Segmentation-Survey\Real-time-Semantic-Segmentation-Survey-main\output_pretrained_Future3_deepseek\xaiacc"
    ...
    # Calculate XAI improvement
    original_acc, corrected_acc, improvement = calculate_xai_improvement(model, images, masks)
    print(f"\nXAI Improvement Analysis:")
    print(f"Original Accuracy: {original_acc:.4f}")
    print(f"Corrected Accuracy: {corrected_acc:.4f}")
